...
def select_entry(entry):
    ...
    if prev is None:
        ...
        logging.info(kmtogo.get() + "km to go  -  Inserted new entry: " + str(entry))
    else:
        return prev
    conn.close()
...
def select_entries(entry):
    ...
    if len(entries) > 1:
        for e in entries:
            print(e)
            logging.info(kmtogo.get() + "km to go  -  Entry: " + str(e))
        return entries
    conn.close()
...
def add_column(table_name, column_name, column_data_type):
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    try:
        c.execute(f"ALTER TABLE {table_name} ADD COLUMN {column_name} {column_data_type}")
        print(f"{column_name} column added successfully to {table_name} table.")
