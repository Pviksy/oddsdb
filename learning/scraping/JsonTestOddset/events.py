...
def get_all():
    url = "https://content.sb.danskespil.dk/content-service/api/v1/q/drilldown-tree"

    querystring = {"drilldownNodeIds": "660", "eventState": "OPEN_EVENT"}
    ...
    # extract id and name from innermost drilldownNodes list
    drilldown_nodes = data["data"]["drilldownNodes"][0]["drilldownNodes"][0]["drilldownNodes"][0]["drilldownNodes"]
    result = {node["id"]: node["name"] for node in drilldown_nodes}
...
def get_race_odds(id):
    ...
    querystring = {"eventSortsIncluded": "TNMT", "includeChildMarkets": "true", "drilldownTagIds": id}
    ...
    outcomes = data["data"]["events"]

    for outcome in outcomes:
        if outcome is None:
            outcomes.remove(outcome)

    #races = outcomes[0]["markets"][0]["outcomes"]
